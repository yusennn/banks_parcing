...
pdf_file_path = "cc_bank_pdfs/center_kz.pdf"
...
print(card)
# Счет
acc_re = r'Түрі\s+(\w+)\s+(\w+)'
acc = re.search(acc_re, pdf_text).group(0)
acc = ' '.join(acc.split())
print(acc)
# Период
per_re = r'Кезеңі\s+\d{2}\.\d{2}\.\d{4}\s-\s\d{2}\.\d{2}.\d{4}'
...
with open('cc_bank_jsons/kz.json', 'w', encoding='utf-8') as json_file:
